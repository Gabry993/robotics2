...
class BasicThymio:
    ...
    prox_sensors_turning = {'thymio14/proximity_right_link': R, 'thymio14/proximity_center_right_link': R, 'thymio14/proximity_center_link': R, 'thymio14/proximity_left_link': L,
                            'thymio14/proximity_center_left_link': L}
    prox_sensors_angle = {'thymio14/proximity_right_link': 40, 'thymio14/proximity_center_right_link': 20, 'thymio14/proximity_center_link': 0, 'thymio14/proximity_left_link': 40,
                          'thymio14/proximity_center_left_link': 20}
    prox_sensors_counter = {'thymio14/proximity_right_link': 0, 'thymio14/proximity_center_right_link': 0, 'thymio14/proximity_center_link': 0, 'thymio14/proximity_left_link': 0,
                            'thymio14/proximity_center_left_link': 0}
    prox_sensors_pointed = {'thymio14/proximity_right_link': False, 'thymio14/proximity_center_right_link': False, 'thymio14/proximity_center_link': False, 'thymio14/proximity_left_link': False,
                            'thymio14/proximity_center_left_link': False}
    ...
    def check_transition(self, data):
        if data.range < 0.08 and self.state == WALKING:
            ...
            self.wall_points.append((data.range * cos(radians(self.prox_sensors_angle[data.header.frame_id])) * self.prox_sensors_turning[data.header.frame_id],
                                     data.range * sin(radians(self.prox_sensors_angle[data.header.frame_id])) * self.prox_sensors_turning[data.header.frame_id]))
    ...
    def move2goal(self, moves):
        ...
        while self.get_distance(goal_pose.position.x, goal_pose.position.y) >= distance_tolerance:
            ...
            vel_msg.angular.x = 0
            vel_msg.angular.y = 0
            vel_msg.angular.z = self.angle_controller.step(angle_difference(atan2(goal_pose.position.y - self.current_pose.position.y, goal_pose.position.x - self.current_pose.position.x), self.yaw),
                                                           self.dt)
